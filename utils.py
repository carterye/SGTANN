...
def save_result(args, result, m):
    result['predict_mode'] = m
    ...
    path = get_project_path() + '/predict/result/' + args.model_name + '.csv'
...
def sliding_window(args, data, ts, te, m):
    window_s = ts - m + 1 - args.window_size
    window_e = ts - m + 1
    target = ts
...
def get_dataset(args, m):
    print(args.dataset_name, m)
    ...
    # 滑动窗口
    train_set = sliding_window(args, data, args.window_size + m - 1, a, m)
    valid_set = sliding_window(args, data, a, b, m)
    test_set = sliding_window(args, data, b, c, m)
...
def train(args, model, dataset, m):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print('\ntotal_loss:', round(float(loss_total) / count, 4), round(min_loss, 4), round(cur_test_loss, 4))
        print("Elapsed time: %.2f seconds." % elapsed_time)


def test(args, model, dataset, m):
    train_set, valid_set, test_set, scale = dataset
    x_test, y_test = test_set
    path = get_project_path() + '/model_saved/' + args.model_name + '/' + args.dataset_name + '/' + str(
        m) + '/weights.ckpt'
    ...
    test_result = evaluate(y_test, y_test_pred, scale)
    save_result(args, test_result, m)
